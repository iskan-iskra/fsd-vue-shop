...
<template>
  <div class="wrapper">
    <div class="error-title">ОШИБКА 404</div>
...
$tail-interval: calc((11 - 7.5) / $tail-wags);

@mixin effect($name, $kf) {
  @keyframes #{$name} {
    @each $interval, $transform in $kf {
      #{percentage(math.div($interval,$total-kf))} {
        transform: $transform;
      }
    }
  }
...
.main {
  ...
  .stand {
    ...
    &::after {
      ...
      box-shadow: 0 10px 0 $cream, 0 20px 0 $cream,
        0 30px 0 $cream, 0 40px 0 $cream, 0 50px 0 $cream,
        0 60px 0 $cream, 0 70px 0 $cream, 0 80px 0 $cream,
        0 90px 0 $cream, 0 100px 0 $cream, 0 110px 0 $cream,
        0 120px 0 $cream, 0 130px 0 $cream, 0 140px 0 $cream,
        0 150px 0 $cream, 0 160px 0 $cream, 0 170px 0 $cream;
    }
  }
  .cat {
    width: 110px;
